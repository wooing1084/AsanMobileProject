...
class SensorChartActivity : AppCompatActivity() {
    ...
    val ppgGreenBufferQueue = ArrayDeque<Entry>()
    val heartBufferQueue = ArrayDeque<Entry>()
...
//    private var ppgGIndex: Float = 0F
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        ppgGreenBufferChart = binding.chartNowPpgGreen
        heartRateBufferChart = binding.chartNowHeart
    ...
    private fun makeBufferChart(){
        ppgGreenBufferChart.clear()
        heartRateBufferChart.clear()

        val ppgFileName = CsvController.getExistFileName(this, "HeartRate")
        val heartFileName = CsvController.getExistFileName(this, "PpgGreen")

        if(ppgFileName == null || heartFileName == null) {
            Log.d(TAG, "No buffered file.")
            return
        }

        val ppgFile = CsvController.getFile(CsvController.getExternalPath(this,"sensor")+ "/" + ppgFileName)
        val heartFile = CsvController.getFile(CsvController.getExternalPath(this,"sensor") + "/" + heartFileName)
...
//            ppgGreenQueue.add(Entry(time, data))
        ...
        ppgGreenBufferChart.data = ppgGreenData
        heartRateBufferChart.data = heartData

        ppgGreenBufferChart.invalidate()
        heartRateBufferChart.invalidate()
